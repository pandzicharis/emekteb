...
export class LekcijaController {
  ...
  @Post()
  create(@Body() body: { naslov: string; opis: string; tezina: number; redoslijed: number; aktivan: boolean; tip: TipLekcije }) {
  ...
  update(
    @Param('id') id: string,
    @Body() body: Partial<{ naslov: string; opis: string; tezina: number; redoslijed: number; aktivan: boolean; tip: TipLekcije }>
  ) {
