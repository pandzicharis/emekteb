...
type TabType = 'podaci' | 'ocjene' | 'prisustvo' | 'skola-hifza';
...
export default function UcenikDetailsDrawer({ open, ucenikId, onClose, onSave }: Props) {
  ...
  // For Škola Hifza tab
  const [isSkolaHifzaUcenik, setIsSkolaHifzaUcenik] = useState(false);
  const [skolaHifzaNapredak, setSkolaHifzaNapredak] = useState<{
    napredak: { [suraName: string]: number[] } | null;
  } | null>(null);
  const [skolaHifzaLekcije, setSkolaHifzaLekcije] = useState<Array<{ id: string; naslov: string; brojAjeta?: number }>>([]);
  const [loadingSkolaHifza, setLoadingSkolaHifza] = useState(false);

  // Fetch ucenik details
  useEffect(() => {
    if (!open || !ucenikId) {
      setUcenik(null);
      setIsSkolaHifzaUcenik(false);
      return;
    }
    ...
    const fetchUcenik = async () => {
      ...
      try {
        ...
        // Provjeri da li je učenik u SKOLA_HIFZA razredu
        // Provjeravamo preko grupa u aktivnoj nastavnoj godini
        try {
          const dashboardRes = await axios.get(`${API_URL}/muallimi/dashboard`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          const razredi = dashboardRes.data?.razredi || [];
          const skolaHifzaRazred = razredi.find(
            (r: any) => r.razred?.ilmihal === 'SKOLA_HIFZA' || r.razred?.ilmihal === 'ŠKOLA HIFZA'
          );
          if (skolaHifzaRazred) {
            const isInHifza = skolaHifzaRazred.grupe?.some((g: any) =>
              g.ucenici?.some((u: any) => u.id === ucenikId)
            );
            setIsSkolaHifzaUcenik(!!isInHifza);
          }
        } catch (err) {
          console.warn('Error checking SKOLA_HIFZA membership:', err);
          setIsSkolaHifzaUcenik(false);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        if (dashboardRes.data?.nastavnaGodina) {
          ...
          if (activeTab === 'ocjene' && ucenik) {
            // Pronađi razred učenika iz dashboard podataka
            const razredi = dashboardRes.data?.razredi || [];
            let razredId: string | null = null;
            
            // Pokušaj pronaći razred učenika preko grupe
            for (const razred of razredi) {
              const grupa = razred.grupe?.find((g: any) => 
                g.ucenici?.some((u: any) => u.id === ucenikId)
              );
              if (grupa && razred.razred?.id) {
                razredId = razred.razred.id;
                break;
              }
            }
            
            // Ako nije pronađen preko grupe, pokušaj pronaći po razredNaziv
            if (!razredId && ucenik.razredNaziv) {
              const razred = razredi.find((r: any) => 
                r.razred?.name === ucenik.razredNaziv || 
                r.razred?.naziv === ucenik.razredNaziv
              );
              if (razred?.razred?.id) {
                razredId = razred.razred.id;
              }
            ...
            // Učitaj lekcije za razred ako je razredId pronađen
            if (razredId) {
              ...
              // Ako nije pronađen razredId, učitaj sve lekcije (fallback)
              console.warn('RazredId nije pronađen za učenika, učitavam sve lekcije');
              const lekcijeRes = await axios.get(`${API_URL}/lekcije`, {
                headers: { Authorization: `Bearer ${token}` },
              });
              setLekcijeForRazred(lekcijeRes.data || []);
  ...
  // Fetch Škola Hifza data when on skola-hifza tab
  useEffect(() => {
    if (activeTab !== 'skola-hifza' || !ucenikId || !isSkolaHifzaUcenik) {
      setSkolaHifzaNapredak(null);
      return;
    }

    const fetchSkolaHifzaData = async () => {
      setLoadingSkolaHifza(true);
      try {
        const token = localStorage.getItem('token');
        ...
        setSkolaHifzaLekcije((lekcijeRes.data || []).sort((a: any, b: any) => a.naslov.localeCompare(b.naslov)));

        // Fetch nastavna godina
        const dashboardRes = await axios.get(`${API_URL}/muallimi/dashboard`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const nastavnaGodinaId = dashboardRes.data?.nastavnaGodina?.id;
        if (nastavnaGodinaId) {
          ...
          try {
            ...
            if (skolaHifzaId) {
              ...
              setSkolaHifzaNapredak(napredakRes.data);
            }
          } catch (err) {
            console.warn('Error fetching SkolaHifza napredak:', err);
            setSkolaHifzaNapredak(null);
          }
        }
      } catch (err) {
        console.error('Error fetching Škola Hifza data:', err);
        setSkolaHifzaNapredak(null);
      } finally {
        setLoadingSkolaHifza(false);
      }
    };

    fetchSkolaHifzaData();
  }, [activeTab, ucenikId, isSkolaHifzaUcenik]);
  ...
  const filteredLekcijeForOcjena = useMemo(() => {
    // Filtriraj lekcije koje su već ocijenjene
    const gradedLessonIds = new Set<string>();
    ocjeneWithLekcije.forEach((ocjena) => {
      gradedLessonIds.add(ocjena.lekcija.id);
    });
    
    // Filtriraj lekcije koje nisu ocijenjene
    const ungradedLessons = lekcijeForRazred.filter((l) => !gradedLessonIds.has(l.id));
    
    const term = lekcijaSearch.trim().toLowerCase();
    if (!term) return ungradedLessons;
    return ungradedLessons.filter((l) => {
  ...
  }, [lekcijeForRazred, lekcijaSearch, ocjeneWithLekcije]);
            ...
            {nastavnaGodina && (
              <div className="w-full">
                ...
                <WeekendDatePicker
                  ...
                  singleColumn={true}
                  maxDate={(() => {
                    // Postavi maxDate na jučerašnji dan (ili današnji dan ako je vikend i dozvoljen)
                    const today = new Date();
            ...
            {/* Submit button */}
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  // Refresh učenik (da se ažurira prosjek i ostali podaci)
                  const ucenikRes = await axios.get<Ucenik>(`${API_URL}/ucenici/${ucenikId}`, {
                    headers: { Authorization: `Bearer ${token}` },
                  });
                  setUcenik(ucenikRes.data);
                  setEditedUcenik(ucenikRes.data);
                  ...
                  // Pozovi onSave callback ako postoji (da se refresh-uje parent komponenta)
                  if (onSave) {
                    await onSave();
                  }
                } catch (err: unknown) {
  ...
  const renderContent = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {activeTab === 'skola-hifza' && renderSkolaHifzaTab()}
      </div>
    );
  };

  const renderSkolaHifzaTab = () => {
    if (!currentUcenik || !isSkolaHifzaUcenik) {
      return (
        <div className="text-center py-8 text-gray-500">
          Učenik nije polaznik Škole Hifza.
        </div>
      );
    }

    if (loadingSkolaHifza) {
      return (
        <div className="flex items-center justify-center py-12">
    ...
    const napredak = skolaHifzaNapredak?.napredak || null;
    
    // Izračunaj statistike
    const ukupnoAjeta = napredak
      ? Object.values(napredak).reduce((sum, ajeta) => sum + ajeta.length, 0)
      : 0;
    const ukupnoAjetaMax = skolaHifzaLekcije.reduce((sum, lekcija) => sum + (lekcija.brojAjeta || 0), 0);
    const procenat = ukupnoAjetaMax > 0 ? Math.round((ukupnoAjeta / ukupnoAjetaMax) * 100) : 0;
    const nauceneSure = napredak ? Object.keys(napredak).length : 0;
    const ukupnoSure = skolaHifzaLekcije.length;
          ...
          <div className="p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-purple-50 rounded-lg border border-purple-200 p-4">
                <div className="text-xs font-medium text-purple-700 mb-1">Naučeno ajeta</div>
                <div className="text-2xl font-bold text-purple-900">{ukupnoAjeta}</div>
                <div className="text-xs text-purple-600 mt-1">od {ukupnoAjetaMax}</div>
              </div>
              <div className="bg-purple-50 rounded-lg border border-purple-200 p-4">
                <div className="text-xs font-medium text-purple-700 mb-1">Naučene sure</div>
                <div className="text-2xl font-bold text-purple-900">{nauceneSure}</div>
                <div className="text-xs text-purple-600 mt-1">od {ukupnoSure}</div>
              </div>
              <div className="bg-purple-50 rounded-lg border border-purple-200 p-4">
                <div className="text-xs font-medium text-purple-700 mb-1">Napredak</div>
                <div className="text-2xl font-bold text-purple-900">{procenat}%</div>
                <div className="text-xs text-purple-600 mt-1">ukupno</div>
              </div>
              <div className="bg-purple-50 rounded-lg border border-purple-200 p-4">
                <div className="text-xs font-medium text-purple-700 mb-1">Preostalo</div>
                <div className="text-2xl font-bold text-purple-900">{ukupnoAjetaMax - ukupnoAjeta}</div>
                <div className="text-xs text-purple-600 mt-1">ajeta</div>
              </div>
            </div>
            <div className="mt-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="p-6">
            {!napredak || Object.keys(napredak).length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                Nema zabilježenog napretka.
              </div>
            ) : (
              <div className="space-y-3">
                {Object.entries(napredak)
                  .sort(([a], [b]) => {
                    const indexA = skolaHifzaLekcije.findIndex((l) => l.naslov === a);
                    const indexB = skolaHifzaLekcije.findIndex((l) => l.naslov === b);
                    return indexA - indexB;
                  })
                  .map(([sura, ajeta]) => {
                    const lekcija = skolaHifzaLekcije.find((l) => l.naslov === sura);
                    const maxAjeta = lekcija?.brojAjeta || 0;
                    const procenatSure = maxAjeta > 0 ? Math.round((ajeta.length / maxAjeta) * 100) : 0;
                    return (
                      <div key={sura} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium text-gray-900">{sura}</span>
                          <span className="text-sm text-gray-600">
                            {ajeta.length} / {maxAjeta} ajeta ({procenatSure}%)
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                          <div
                            className="bg-purple-600 h-2 rounded-full transition-all"
                            style={{ width: `${procenatSure}%` }}
                          ></div>
                        </div>
                        <div className="text-xs text-gray-500">
                          Naučeni ajeta: {ajeta.sort((a, b) => a - b).join(', ')}
                        </div>
                      </div>
                    );
                  })}
      ...
      >
        ...
        <div className="border-b border-gray-200 bg-gray-50 px-6">
          <div className="flex gap-1">
            ...
            {isSkolaHifzaUcenik && (
              <button
                onClick={() => setActiveTab('skola-hifza')}
                className={`px-4 py-3 text-sm font-medium transition-colors border-b-2 ${
                  activeTab === 'skola-hifza'
